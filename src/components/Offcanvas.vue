<template>
  <div
    ...
    class="offcanvas offcanvas-end bg-info overflow-y-scroll"
    tabindex="-1"
    id="offcanvasRight"
    aria-labelledby="offcanvasRightLabel"
  >
    <div class="offcanvas-header bg-success flex-column transition position-relative mb-4">
      <div class="d-flex justify-content-between w-100 mb-3">
        <div>
          <button
            @click="removeSidebarOpenBtnClass"
            type="button"
            class="btn text-white hvr-pulse-grow p-0 me-2"
      ...
      <div
        ref="searchBar"
        class="transition overflow-hidden w-100 height-352 transform mb-2"
      >
      ...
      <div class="position-absolute top-100 start-50 translate-middle z-index-999">
        <button
          @click.stop="dropSearchBar"
          type="button"
          class="btn dropSearchBarBtn transition"
        >
          <i class="bi bi-arrow-up-circle-fill fs-3 text-secondary"></i>
        </button>
      </div>
    </div>
  ...
  methods: {
    ...
    search() {
      if (!this.pharmacySelect.length) {
        this.pharmacySelect = ['none', 'low', 'full'];
      }
      this.$emit('search', {
    ...
    dropSearchBar() {
      this.$refs.searchBar.classList.toggle('height-0');
      this.$refs.searchBar.classList.toggle('height-352');
      document.querySelector('.dropSearchBarBtn').classList.toggle('rotate-180');
    },
    toMyLocation() {
